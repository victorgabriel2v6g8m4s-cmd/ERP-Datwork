...
export function UniversalSearchBar({
    ...
    const hasCatalogFilters = isProducts || type === 'ingredients';
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 items-end w-full">
                            <div className="space-y-1.5 flex-1 min-w-[240px] w-full">
                                ...
                                >
                                    ...
                                    {!isRecipes && !isAgenda && (
                                        <option value="value">
                                            {isProducts ? 'Custo total unitário (Maior)' : isExpenses ? 'Valor (Maior para Menor)' : 'Preço de custo (Maior)'}
                                        </option>
                                    )}
                                    {isProducts && <option value="rating">Avaliações e Notas (Melhores)</option>}
                                    {hasCatalogFilters && orderProfiles?.map((profile) => (
                                        <option key={profile.id} value={`profile-${profile.id}`}>Perfil: {profile.name}</option>
                                    ))}
                                </select>
                            </div>
                            ...
                            {hasCatalogFilters && (
                                <div className="space-y-1.5 flex-1 min-w-[240px] w-full">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => onFilterChange({ ...filters, search: '', sortBy: 'custom', abcCategory: 'all', unitFilter: 'all', statusFilter: 'all', dateFilter: 'all' })}
